    ...
    >
      <div className="flex items-center gap-3">
        ...
        <div className="flex flex-col">
          ...
          <div className="text-xs text-gray-600">{(data as any)?.title || 'Output'}</div>
        </div>
      </div>

      {(data as any)?.outputs && Object.keys((data as any).outputs).length > 0 && (
        <div className="mt-2 pt-2 border-t border-red-200">
          <div className="flex items-center gap-1 text-xs text-gray-600">
            <FileOutput className="w-3 h-3" />
            <span>Outputs: {Object.keys((data as any).outputs).length}</span>
          </div>
          <div className="mt-1 space-y-1">
            {Object.keys((data as any).outputs).slice(0, 2).map(key => (
