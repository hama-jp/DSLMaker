...
export function VariableAggregatorNodeProperties({ node, onUpdate }: VariableAggregatorNodePropertiesProps) {
  ...
  const handleAggregationModeChange = useCallback((value: string) => {
    // Note: aggregation_mode is not part of standard DifyNode data structure
    // onUpdate({ aggregation_mode: value })
    console.log('Aggregation mode change not implemented:', value)
  }, [onUpdate])

  const handleOutputVariableChange = useCallback((value: string) => {
    // Note: output_variable is not part of standard DifyNode data structure
    // onUpdate({ output_variable: value })
    console.log('Output variable change not implemented:', value)
  }, [onUpdate])

  const handleWeightChange = useCallback((inputKey: string, weight: string) => {
    // Note: weight_config is not part of standard DifyNode data structure
    ...
    console.log('Weight change not implemented:', inputKey, weight)
  }, [data, onUpdate])
  ...
  const handleAddInput = useCallback(() => {
    ...
    // }
    // onUpdate({ weight_config: newConfig })
    console.log('Add input not implemented:', newInputKey, newInputWeight)
    setNewInputKey('')
    setNewInputWeight('1')
  ...
  const handleRemoveInput = useCallback((inputKey: string) => {
    ...
    // onUpdate({ weight_config: newConfig })
    console.log('Remove input not implemented:', inputKey)
  }, [data, onUpdate])
