...
class PerformanceMonitor {
  ...
  generateReport(operationId?: string): string {
    ...
    metrics.forEach((metric, index) => {
      ...
      report += `├─ 🧠 Memory: ${(metric.memoryUsage?.used / 1024 / 1024).toFixed(2) || 'N/A'}MB (${metric.memoryUsage?.percentage?.toFixed(1) || 'N/A'}%)\n`;
      report += `├─ 🔗 Nodes: ${metric.workflowStats?.nodeCount || 0}\n`;
      report += `├─ 🔄 Iterations: ${metric.workflowStats?.iterationNodeCount || 0}\n`;
      report += `├─ 📈 Complexity Score: ${metric.workflowStats?.complexityScore || 0}\n`;
      report += `└─ 📅 Timestamp: ${metric.systemInfo?.timestamp ? new Date(metric.systemInfo.timestamp).toLocaleString() : 'N/A'}\n\n`;
    });
    ...
    if (metrics.length > 1) {
      ...
      const avgMemory = metrics.reduce((sum, m) => sum + (m?.memoryUsage?.used || 0), 0) / metrics.length;
      const avgComplexity = metrics.reduce((sum, m) => sum + (m?.workflowStats?.complexityScore || 0), 0) / metrics.length;
