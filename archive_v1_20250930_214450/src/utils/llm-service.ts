...
import { scaffoldWorkflowVariables, VariableDictionary } from './variable-scaffolder'
...
export class LLMService {
  ...
  async generateDSL(userRequirement: string): Promise<DSLGenerationResult> {
    try {
      ...
      // Parse content - try YAML first (Dify's native format), then JSON
      let dslObject: any
      let isYamlFormat = false

      // Check if content looks like YAML (more comprehensive detection)
      const looksLikeYaml = cleanContent.startsWith('app:') ||
                           cleanContent.startsWith('workflow:') ||
                           cleanContent.startsWith('kind:') ||
                           /^\w+:\s*[\w\[\{]/.test(cleanContent)

      if (looksLikeYaml) {
        // Try parsing as YAML first
        try {
          dslObject = yaml.load(cleanContent) as any
          isYamlFormat = true
          console.log('✅ Successfully parsed YAML response from LLM')
        } catch (yamlError) {
          return {
            success: false,
            error: `Invalid YAML from LLM: ${yamlError instanceof Error ? yamlError.message : String(yamlError)}\n\nContent preview: ${cleanContent.substring(0, 200)}...`,
            llmResponse
          }
        }
      } else {
        // Try parsing as JSON
        try {
          dslObject = JSON.parse(cleanContent)
          console.log('✅ Successfully parsed JSON response from LLM')
        } catch (jsonError) {
          // If JSON fails, try YAML as fallback
          try {
            dslObject = yaml.load(cleanContent) as any
            isYamlFormat = true
            console.log('✅ Successfully parsed YAML response from LLM (JSON fallback)')
          } catch (yamlError) {
            return {
              success: false,
              error: `Invalid format from LLM. Neither JSON nor YAML could be parsed.\nJSON error: ${jsonError instanceof Error ? jsonError.message : String(jsonError)}\nYAML error: ${yamlError instanceof Error ? yamlError.message : String(yamlError)}\n\nContent preview: ${cleanContent.substring(0, 200)}...`,
              llmResponse
            }
          }
        }
      ...
      if (dslObject.workflow?.graph?.edges && Array.isArray(dslObject.workflow.graph.edges)) {
        dslObject.workflow.graph.edges.forEach((edge: any, index: number) => {
          if (edge && typeof edge === 'object' && !edge.id) {
            edge.id = `edge-${index + 1}-${edge.source || 'unknown'}-${edge.target || 'unknown'}`
          }
        })
      }
      ...
      try {
        ...
        return {
          success: false,
          error: `YAML conversion failed: ${yamlError instanceof Error ? yamlError.message : String(yamlError)}`,
          llmResponse
        }
      }
      ...
      const { workflow: scaffoldedWorkflow, dictionary } = scaffoldWorkflowVariables(parseResult.workflow, analysis)

      const finalYamlContent = yaml.dump(scaffoldedWorkflow, {
        indent: 2,
        lineWidth: -1,
        noRefs: true,
        quotingType: '"',
      ...
      const lintResult = lintDSL(scaffoldedWorkflow)

      return {
        success: true,
        dsl: scaffoldedWorkflow,
  ...
  async modifyDSL(
    ...
    try {
      ...
      const { workflow: scaffoldedWorkflow, dictionary } = scaffoldWorkflowVariables(parseResult.workflow)
  ...
  async generateDSLStream(
    ...
    try {
      ...
      try {
        ...
        return {
          success: false,
          error: `Invalid JSON from streamed response: ${jsonError instanceof Error ? jsonError.message : String(jsonError)}\n\nContent preview: ${jsonContent.substring(0, 200)}...`
        }
      }
      ...
      // Fix missing edge IDs (same as generateDSL)
      if (dslObject.workflow?.graph?.edges && Array.isArray(dslObject.workflow.graph.edges)) {
        dslObject.workflow.graph.edges.forEach((edge: any, index: number) => {
          if (edge && typeof edge === 'object' && !edge.id) {
            edge.id = `edge-${index + 1}-${edge.source || 'unknown'}-${edge.target || 'unknown'}`
...
// Iteration item to processing node
{
  "source": "iteration-1",
  "target": "llm-process",
...
// Processing result back to iteration
{
  "source": "llm-process",
  "target": "iteration-1",
  "sourceHandle": "output",
