...
export function parseDSL(yamlContent: string): DSLParseResult {
  ...
  try {
    // Parse YAML and clone to remove readonly properties
    const parsed = yaml.load(yamlContent) as unknown

    // Deep clone the parsed object to remove any readonly properties
    const clonedParsed = JSON.parse(JSON.stringify(parsed))

    // Validate basic structure
    if (!clonedParsed || typeof clonedParsed !== 'object') {
      errors.push('Invalid YAML format: Root must be an object')
      return { success: false, errors }
    }

    const data = clonedParsed as Record<string, unknown>
    ...
    const dslFile = data as unknown as DifyDSLFile
...
/**
 * Detect cycles in the workflow graph (excluding valid iteration loop-backs)
 */
function detectCycles(nodes: DifyDSLFile['workflow']['graph']['nodes'], edges: DifyDSLFile['workflow']['graph']['edges']) {
  ...
  // Find iteration nodes
  const iterationNodes = new Set(
    nodes.filter(node => node.type === 'iteration').map(node => node.id)
  )

  edges.forEach(edge => {
    // Skip iteration loop-back edges (these are valid structural loops)
    const isIterationLoopBack =
      iterationNodes.has(edge.target) &&
      (edge as any).targetHandle === 'result_input' &&
      (edge as any).data?.isInIteration === true

    // Only add non-iteration-loopback edges to cycle detection
    if (!isIterationLoopBack) {
...
function validateWorkflowConnectivity(nodes: DifyDSLFile['workflow']['graph']['nodes'], edges: DifyDSLFile['workflow']['graph']['edges']): string[] {
  ...
  nodes.forEach(node => {
    ...
    // All other nodes should have both, with special cases
      ...
      if ((node.type as string).includes('if-else') || (node.type as string).includes('if_else')) {
      ...
      if ((node.type as string).includes('variable-aggregator') || (node.type as string).includes('variable_aggregator') ||
          (node.type as string).includes('variable-assigner') || (node.type as string).includes('variable_assigner')) {
  ...
  switch (node.type) {
    ...
    case NODE_TYPES.LLM:
      ...
      if (!node.data.model) {
        ...
        const model = node.data.model as unknown as Record<string, unknown>
