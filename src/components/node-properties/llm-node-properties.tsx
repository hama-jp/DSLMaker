...
export function LLMNodeProperties({ node, onUpdate }: LLMNodePropertiesProps) {
  ...
  const handlePromptChange = useCallback((value: string) => {
    const messages = data.prompt_template?.messages || []
      ...
      onUpdate({
        prompt_template: {
          ...
          messages: messages.map(msg =>
            msg.role === 'system' ? { ...msg, text: value } : msg
          )
        }
      })
    } else {
  ...
  const systemPrompt = data.prompt_template?.messages?.find(msg => msg.role === 'system')?.text || ''
