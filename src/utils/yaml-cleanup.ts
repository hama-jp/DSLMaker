...
export function validateYamlSyntax(yamlContent: string): { isValid: boolean; error?: string } {
  try {
    // Try to load js-yaml, fallback to basic validation if not available
    let yaml: any
    try {
      yaml = require('js-yaml')
    } catch {
      // If js-yaml is not available, do basic validation
      return {
        isValid: !yamlContent.includes('[invalid') && yamlContent.trim().length > 0,
        error: undefined
      }
    }
